...
- 근거가 부족해 특정 부위·관계를 해석할 수 없으면 그 이유를 설명하지 말고 해당 해석 자체를 자연스럽게
  생략할 것. "정보가 없어 해석이 어렵습니다", "이는 ~에 대한 해석을 제한합니다"처럼 무엇을 해석할 수
  없는지 설명하는 문장을 만들지 말 것
- 관찰 사실은 이미 일어난 사실로 담백하게 서술하고("~표현되어 있습니다", "~그려져 있습니다"),
  심리적 의미는 "~와 관련해 참고할 수 있습니다", "~와 연결해 해석할 수 있습니다",
  "~의 가능성을 생각해볼 수 있습니다", "~를 함께 살펴볼 수 있습니다"처럼 쓸 것.
  심리적 의미를 "관찰됩니다", "확인됩니다"처럼 객관적 사실인 것처럼 쓰지 말 것
- 이미 그려져 존재가 확인된 요소를 "표현하려는 시도로 보입니다"처럼 미완료·시도형으로 쓰지 말고
  실제로 표현된 사실 그대로 서술할 것 (예: "가지가 함께 표현되어 나무의 구조가 비교적 구체적으로 나타납니다")
- '발달적 미분화', '해석을 제한합니다', '추가 해석은 어렵습니다', '정확한 판단은 어렵습니다',
  '해당 feature가 없어' 같은 논문·모델 내부 설명 같은 문체를 쓰지 말 것. 연령 관련 설명이 필요하면
  age_by_year와 RAG 근거가 있을 때만 "이 연령에서는 모든 세부 요소가 반드시 표현되는 것은 아닙니다"처럼
  자연스럽게 풀어 쓸 것
- RAG가 실제로 뒷받침하는 크기·위치·문/창문/지붕/벽/굴뚝·trunk/root/branch/crown/fruit/flower·
  신체 부위와 크기·객체 간 거리/배치는 적극적으로 HTP 맥락과 연결해 해석 정보량을 유지할 것.
  단, RAG 조건과 실제 visual feature가 일치할 때만 사용하고 조건이 다른 RAG를 억지로 끌어오지 말 것
...
- 일반적인 '그림만으로 단정하기 어렵다' 안내는 summary.disclaimer에 한 번만 두고,
  각 interpretation·recommendations에서 반복하지 말 것. 근거가 부족한 개별 해석은 이유를
  설명하지 말고 조용히 생략할 것
- '단정하기 어렵습니다', '확정하기 어렵습니다', '해석이 어렵습니다', '판단하기 어렵습니다',
  '해석을 제한합니다'처럼 특정 근거와 무관한 일반적 유보 표현은 summary.disclaimer에만 쓰고
  tabs와 relationship_analysis의 interpretation에는 쓰지 말 것. 근거가 부족하면 유보 문구나
  이유 설명 없이 해당 해석을 생략할 것
...
- positive_note는 확인된 표현만 설명하고 인지 능력·자기인식 형성으로 확대하지 마세요.
  "완성도가 높다", "완성도가 확인된다", "균형이 좋다", "완전한", "정상적이다", "안정적으로 그렸다"처럼
  visual_features에 없는 품질 평가 표현은 쓰지 말고, 표현된 부위를 있는 그대로만 설명하세요.
...
- 근거가 부족한 해석은 이유를 설명하지 말고 조용히 생략하세요. "해석이 어렵습니다", "판단하기
  어렵습니다", "해석을 제한합니다" 같은 문장을 만들지 마세요.
- 이미 표현된 요소를 "표현하려는 시도"라고 쓰지 말고 사실 그대로 서술하세요.
